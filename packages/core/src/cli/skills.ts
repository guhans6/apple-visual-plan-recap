...
Examples:
  ...
  npx @agent-native/skills@latest add --client codex --scope project
...
flow with the bundled Agent-Native public skills catalog enabled. Pass
--no-connect to register MCP where possible without authenticating (leave auth
to the host or run "npx @agent-native/core@latest connect" later). Pass
--mcp-url to register that connector against a custom origin (an ngrok tunnel,
a local dev server, or a self-hosted deployment) instead of the built-in hosted
default — a bare origin gets the standard /_agent-native/mcp path appended.
Use app-skill pack for marketplace bundles and custom adapter output. Explicit
legacy "BuilderIO/skills" inputs remain supported as a compatibility path during
migration.
...
- Write the plan as a local MDX folder: use
  \`docs/visual-companion/plans/<slug>/\` when the user wants the artifact
  checked into the repo, or use a repo-ignored/temporary folder such as
  \`.agent-native/plans/<slug>/\` or \`/tmp/agent-native-plans/<slug>/\`
...
- Run \`npx @agent-native/core@latest plan local check --dir docs/visual-companion/plans/<slug>\`
  before serving, then run
  \`npx @agent-native/core@latest plan local serve --dir docs/visual-companion/plans/<slug> --kind plan --open\`.
  Report the returned local bridge URL from stdout or \`docs/visual-companion/plans/<slug>/.plan-url\`.
...
- For headless verification, run
  \`npx @agent-native/core@latest plan local verify --dir docs/visual-companion/plans/<slug> --kind plan\`.
...
- Fetch/read the block catalog before writing structured MDX. Use
  ...
  the renderer schema.
- Write the recap as a local MDX folder: use
  \`docs/visual-companion/recaps/<slug>/\` when the user wants the artifact
  checked into the repo, or use a repo-ignored/temporary folder such as
  \`.agent-native/plans/<slug>/\` or \`/tmp/agent-native-plans/<slug>/\`
  when it should not be checked in. The folder contains \`plan.mdx\`, optional
  ...
  the source.
- Run \`npx @agent-native/core@latest plan local check --dir docs/visual-companion/recaps/<slug>\`
  before serving, then run
  \`npx @agent-native/core@latest plan local serve --dir docs/visual-companion/recaps/<slug> --kind recap --open\`.
  Report the returned local bridge URL from stdout or \`docs/visual-companion/recaps/<slug>/.plan-url\`.
  Treat \`.plan-url\` as a local token file and do not commit it. The URL opens
  the hosted Plan UI but reads from the localhost bridge on this machine, so it
  is not shareable across machines. On macOS, \`--open\` prefers Chromium browsers;
  if Safari opens, switch to Chrome/Chromium because Safari can block the hosted
  ...
  is also valid.
- For headless verification, run
  \`npx @agent-native/core@latest plan local verify --dir docs/visual-companion/recaps/<slug> --kind recap\`.
  It starts the bridge, checks the private-network preflight and JSON payload,
  prints diagnostics, and exits. If the browser hangs on "Loading plan", fetch
  the \`bridgeUrl\` from the verify/serve JSON to read the concrete validation
  error.
...
bridge URL from
\`npx @agent-native/core@latest plan local serve --dir docs/visual-companion/recaps/<slug> --kind recap --open\`
or from \`docs/visual-companion/recaps/<slug>/.plan-url\`. It opens the hosted Plan UI but reads from the
...
\`docs/visual-companion/recaps/<slug>/plan.mdx\` may be mentioned only as secondary source-control
...
recaps as MDX folders under \`docs/visual-companion/plans/<slug>/\` and
\`docs/visual-companion/recaps/<slug>/\` when they should be checked in, or
...
\`npx @agent-native/core@latest plan local check --dir docs/visual-companion/plans/<slug>\`, then
\`npx @agent-native/core@latest plan local serve --dir docs/visual-companion/plans/<slug> --kind plan|recap --open\`,
and report the local bridge URL from stdout or \`docs/visual-companion/plans/<slug>/.plan-url\`. Treat
...
bridge; run \`plan local verify --dir docs/visual-companion/plans/<slug> --kind plan|recap\` for
