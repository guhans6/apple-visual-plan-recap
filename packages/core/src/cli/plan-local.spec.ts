...
async function canBindLocalTcpPort(): Promise<boolean> {
...
const localTcpBindingAvailable = await canBindLocalTcpPort();
const bridgeIt = localTcpBindingAvailable ? it : it.skip;
...
describe("local plan CLI helpers", () => {
  ...
  bridgeIt("starts the bridge through both plan serve and plan local serve", async () => {
  ...
  it("includes a repo-relative path for direct local Plan app routes", () => {
    ...
    const dir = path.join(repo, "docs", "visual-companion", "plans", "checkout");
    writeSamplePlan(dir);
    process.chdir(repo);
    ...
    expect(result.url).toBe(
      "http://localhost:8096/local-plans/checkout?path=docs%2Fvisual-companion%2Fplans%2Fcheckout",
    );
  });

  it("defaults local plan folders to docs/visual-companion/plans inside the target repo", async () => {
    const repo = tmpDir();
    fs.mkdirSync(path.join(repo, ".git"));

    const result = await resolveDefaultLocalPlanDir({
      repoRoot: repo,
      kind: "plan",
    });

    expect(result).toBe(path.join(repo, "docs", "visual-companion", "plans"));
  });

  it("defaults local recap folders to docs/visual-companion/recaps inside the target repo", async () => {
    const repo = tmpDir();
    fs.mkdirSync(path.join(repo, ".git"));
    ...
    expect(result).toBe(path.join(repo, "docs", "visual-companion", "recaps"));
  });
  ...
  bridgeIt("serves a tokened localhost bridge for the hosted local plan UI", async () => {
    const dir = path.join(tmpDir(), "checkout");
    writeSamplePlan(dir);
  ...
  bridgeIt("verifies the localhost bridge headlessly and reports Safari guidance", async () => {
    const dir = path.join(tmpDir(), "checkout");
    writeSamplePlan(dir);
