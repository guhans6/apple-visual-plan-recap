...
function defaultRepoRoot(startDir = process.cwd()): string {
  const manifest = planManifestConfig();
  if (manifest) return path.resolve(manifest.rootDir);
  return localPlanWorkspaceRoot(startDir);
}

export async function resolveDefaultLocalPlanDir(input: {
  repoRoot: string;
  kind: LocalPlanKind;
  explicitDir?: string;
}): Promise<string> {
  if (input.explicitDir?.trim()) {
    return path.resolve(input.repoRoot, input.explicitDir.trim());
  }
  const bootstrap = await ensureVisualCompanionRoot(input.repoRoot);
  return input.kind === "recap"
    ? bootstrap.recapsAbsolute
    : bootstrap.plansAbsolute;
}
...
async function writeLocalPlanSkeleton(input: {
  dir?: string;
  title: string;
  brief?: string;
  kind: LocalPlanKind;
  ...
  const defaultDir = await resolveDefaultLocalPlanDir({
    repoRoot,
    kind: input.kind,
    explicitDir: process.env.PLAN_LOCAL_DIR?.trim(),
  });
  const dir = path.resolve(
    input.dir || path.join(defaultDir, localPlanFolderName(input.title)),
  );
...
async function runInit(args: Record<string, string | boolean>): Promise<void> {
  ...
  const result = await writeLocalPlanSkeleton({
