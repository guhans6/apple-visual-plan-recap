...
Install Agent-Native public skill folders into Codex and Claude skill directories.
...
Skill content is bundled from the Agent-Native public skills catalog that ships
with `@agent-native/skills`, so normal installs no longer depend on a separate
public skills repo checkout. Explicit `visual-plan` / `visual-recap` installs
delegate to `@agent-native/core` so Plan mode selection, MCP registration, and
local-files instructions stay in one framework-owned flow. Explicit
`BuilderIO/skills` inputs still work as a compatibility alias during migration.
