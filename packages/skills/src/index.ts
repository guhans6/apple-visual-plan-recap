...
const LEGACY_PUBLIC_SKILLS_SOURCE = "BuilderIO/skills";
const DEFAULT_SKILLS_SOURCE = path.resolve(
...
export function parseSkillsCliArgs(argv: string[]): ParsedArgs {
  ...
  if (
    out.source &&
    out.source !== LEGACY_PUBLIC_SKILLS_SOURCE &&
  ...
  if (result.planMode === "local-files") {
    clack.note(
      [
        ...
        "Checked-in default: docs/visual-companion/plans/<slug>",
        "Check: npx @agent-native/core@latest plan local check --dir docs/visual-companion/plans/<slug>",
        "Serve: npx @agent-native/core@latest plan local serve --dir docs/visual-companion/plans/<slug> --open",
        "Verify: npx @agent-native/core@latest plan local verify --dir docs/visual-companion/plans/<slug>",
      ].join("\n"),
      "Local Plan files",
    );
  }
...
async function materializeSource(input: string): Promise<{
  ...
  if (input === LEGACY_PUBLIC_SKILLS_SOURCE) {
    return { root: DEFAULT_SKILLS_SOURCE };
  }
