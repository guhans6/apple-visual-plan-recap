...
function bundledCatalogRoot(): string {
  return path.join(workspaceRoot(), "packages", "skills", "catalog");
}
...
describe("@agent-native/skills", () => {
  ...
  it("installs plain skills from the bundled catalog when no source is provided", async () => {
    const project = tmpDir();
    vi.stubGlobal("fetch", async () => {
      throw new Error("bundled catalog should not require a network fetch");
    });
    ...
    expect(result.source).toContain(path.join("packages", "skills", "catalog"));
    expect(result.skills).toEqual(["quick-recap"]);
    expect(
      fs.existsSync(
        path.join(project, ".agents", "skills", "quick-recap", "SKILL.md"),
  ...
  it("maps explicit BuilderIO/skills installs to the bundled catalog", async () => {
    const project = tmpDir();
    vi.stubGlobal("fetch", async () => {
      throw new Error(
        "legacy BuilderIO/skills compatibility should stay local",
      );
    });
    ...
    expect(result.source).toBe(bundledCatalogRoot());
    expect(result.skills).toEqual(["quick-recap"]);
    expect(
      fs.existsSync(
        path.join(project, ".agents", "skills", "quick-recap", "SKILL.md"),
  ...
  it("delegates default visual-plan installs to agent-native core", async () => {
    ...
    expect(runCoreSkills).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        publicSkillEntries: [],
        publicSkillSource: bundledCatalogRoot(),
      }),
    );
  });
