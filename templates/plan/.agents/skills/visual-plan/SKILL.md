...
4. Surface the returned Plans link or inline MCP App and ask the user to review.
   ...
   **Self-Review Before Handoff** while the user reads, instead of blocking the
   handoff on it.
5. Call `get-companion-feedback` before editing a companion artifact, after
   review, after any long pause, and before the final response. Treat the local
...
- `get-companion-plan`: read the current local companion artifact with target
  metadata and local feedback-linked context.
- `get-companion-feedback`: read pending local companion feedback and act only
  on the comments meant for the agent.
...
- `export-companion-selection`: export the specific companion evidence or
  source selection the user asked for.
...
`get-companion-feedback` returns the local feedback queue — read it before
...
- **Detached comments.** When a companion feedback thread no longer maps
  cleanly to current content, reconcile it against the rewritten artifact —
