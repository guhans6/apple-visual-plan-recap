...
`get-companion-feedback` to drive fixes back into the code — annotation → agent →
diff, the same close-the-loop flow forward plans use. After a reviewer annotates
a block, call `get-companion-feedback` to read the local feedback queue, then
either update the recap with `create-visual-recap` (passing the existing
`planId` to replace it in place) or apply targeted local-file changes. The
