...
type PlanContentRendererProps = {
   ...
   * server save lands. Without this the lagging companion read refresh
   * re-supplies the pre-edit content during the save window and the reconcile
   * briefly reverts the new layout ("drop works, then undoes, then comes
   * back"). Called only on structural changes — a per-keystroke optimistic
   * write would make the editor's `value` track local state and loop the
   * reconcile.
   */
