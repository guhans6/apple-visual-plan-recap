...
/**
 * Routes whose page renders its own h-12 toolbar.
 * Layout still wraps these with the left Sidebar but skips the
 * global Header so they don't double-stack a header bar.
 */
function routeOwnsToolbar(pathname: string): boolean {
  return isPlanDetailRoute(pathname);
}

function isPlanDetailRoute(pathname: string): boolean {
  return isCompanionDetailPath(pathname);
}
