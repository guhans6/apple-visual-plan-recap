...
Visual Companion is structured visual artifact mode for coding agents. It turns a
...
walkthroughs and file trees, code previews, annotations, feedback, and HTML
export.

## Install

Use the Agent-Native CLI. This is the recommended setup because it installs the
Plan skill instructions and registers the local Plan MCP connector in one step:
...
4. The agent reads structured feedback before editing and updates the plan or
   implementation.
5. The user updates the local plan or recap, reviews the result in the
   companion, and keeps the files under normal source control.

## Project Companion

The default companion contract is target-repo-first:

- store checked-in artifacts under `docs/visual-companion/`
- keep plans in `docs/visual-companion/plans/`
- keep recaps in `docs/visual-companion/recaps/`
- bootstrap that folder on first requested tool use
- `/` opens the current project's companion dashboard
- keep other sources hidden behind an explicit opt-in section by default

Use `PLAN_LOCAL_DIR` only when you intentionally want an explicit override. The
normal case is to point the companion at the working project, not the companion
app checkout.
...
For the companion workflow, treat the GitHub Action as optional
automation around recap generation. It is not required for local review.
