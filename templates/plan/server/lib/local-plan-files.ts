/**
 * Local file sync for the no-login companion runtime.
 *
 * In companion runtime, created and updated plans are written to the repo as MDX so
...
} from "../../shared/companion-routes.js";
...
const DEFAULT_REPO_PLANS_PATH = `${VISUAL_COMPANION_ROOT}/plans`;
const DEFAULT_VISUAL_COMPANION_README = `# Visual Companion
...
export function localPlansDir(): string {
  ...
  if (configured && configured.trim().length > 0) {
    ...
    return path.isAbsolute(trimmed)
      ...
      : path.resolve(localPlanRepoRoot(), trimmed);
  }
  const root = localPlanRepoRoot();
  const dir = path.resolve(root, DEFAULT_REPO_PLANS_PATH);
  ensureVisualCompanionRootSync(root);
  return dir;
}
...
function ensureVisualCompanionRootSync(repoRoot: string): void {
  const root = path.resolve(repoRoot, VISUAL_COMPANION_ROOT);
  fsSync.mkdirSync(path.join(root, "plans"), { recursive: true });
  fsSync.mkdirSync(path.join(root, "recaps"), { recursive: true });
  const readmePath = path.join(root, "README.md");
  if (!fsSync.existsSync(readmePath)) {
    fsSync.writeFileSync(readmePath, DEFAULT_VISUAL_COMPANION_README, "utf-8");
  }
}
...
function localPlanRoutePath(slug: string, repoPath?: string | null): string {
  return companionRoutePath(inferCompanionRouteKind(repoPath), slug, repoPath);
}
