...
describe("local-plan-files", () => {
  ...
  it("resolves a relative PLAN_LOCAL_DIR from the repo root", () => {
    process.env.PLAN_LOCAL_DIR = "docs/visual-companion/plans";
    process.env.PLAN_REPO_ROOT = tmpDir;

    expect(localPlansDir()).toBe(
      path.join(tmpDir, "docs", "visual-companion", "plans"),
    );
  });

  it("defaults to docs/visual-companion/plans under the repo root when PLAN_LOCAL_DIR is unset", async () => {
    ...
    await expect(defaultLocalPlanRepoPath("checkout-review")).resolves.toBe(
      "docs/visual-companion/plans/checkout-review",
    );
  });
  ...
  it("promotes temporary local plans into the configured repo path", async () => {
    ...
    await writePlanLocalFolder({
      ...
      brief: content.brief,
      content,
      url: "/companion/plans/checkout-review",
    });
    ...
    expect(promoted.promoted.routePath).toBe(
      "/companion/plans/checkout-review?path=docs%2Fplans%2Fcheckout-review",
    );
    await expect(
