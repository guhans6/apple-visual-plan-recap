...
function planUrl(planId: string, kind: PlanKind = "plan"): string {
  const appUrl = getAppProductionUrl().replace(/\/+$/, "");
  const prefix = kind === "recap" ? "/recaps" : "/plans";
  const path = appPath(`${prefix}/${encodeURIComponent(planId)}`);
  try {
...
async function sendPlanCommentNotification(input: {
  ...
  const { html, text } = renderEmail({
    ...
    cta: { label: "Open plan", url: planUrl(input.planId, input.planKind) },
    footer:
      input.recipient.reason === "plan-owner"
