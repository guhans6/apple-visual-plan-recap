...
export function shouldSkipDbMigrationsForLocalCompanion(): boolean {
  return isLocalPlanRuntime();
}

const PLAN_DB_MIGRATIONS = [
    {
      version: 1,
...
export default async function planDbPlugin(nitroApp: unknown) {
  if (shouldSkipDbMigrationsForLocalCompanion()) {
    markDefaultPluginProvided(nitroApp, "db");
    return;
  }

  const { runMigrations } = await import("@agent-native/core/db");
  const plugin = runMigrations(PLAN_DB_MIGRATIONS, {
    table: "plans_migrations",
  });
  return plugin(nitroApp as never);
}
