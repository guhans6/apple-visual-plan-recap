...
describe("Plans helpers", () => {
  ...
  it("detects implementation sections from file-level plans", () => {
    const sections = deriveSectionsFromText(
      "# Implementation\n\n- templates/plan/app/pages/CompanionArtifactEditor.tsx — symbols: `CompanionArtifactEditor`; keep the companion reader shell focused on local artifact review.\n\n```tsx\nfunction CompanionArtifactEditor() {}\n```",
    );
  ...
  it("builds a UI-first hybrid plan with a top canvas and document tabs", () => {
    ...
    expect(html).toContain("Excalifont-Regular.woff2");
  ...
  it("renders file references as previewable implementation tabs", () => {
    ...
    implementation.body =
      "- templates/plan/app/pages/CompanionArtifactEditor.tsx:42 — symbols: `CompanionArtifactEditor`; render the companion artifact reader without reintroducing hosted review chrome.\n\n```tsx\nfunction CompanionArtifactEditor() {\n  return null;\n}\n```";
    const bundle: PlanBundle = {
      plan: {
        id: "plan_1",
    ...
    expect(html).not.toContain("CompanionArtifactEditor.tsx:42");
