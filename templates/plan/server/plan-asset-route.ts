 ...
 *
 * Access control follows the same local plan access rules as the action
 * surface.
 *
 * Cache: long immutable because the asset bytes never change for a given ID.
 */
import {
  defineEventHandler,
  getMethod,
 ...
 * `getSession` handles browser cookies, legacy bearer tokens, and MCP bearer
 * tokens in one call — no custom auth logic needed here.
 */
