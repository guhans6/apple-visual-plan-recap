...
<ImplementationMap
  ...
  files={[
    {
      path: "app/pages/CompanionArtifactEditor.tsx",
      note: "Companion artifact shell / skeleton handling — keep the loading surface stable until the first settled bundle",
    },
    {
      path: "app/hooks/use-companion-plan.ts",
      note: "stop clearing companion content on background refresh so the editor does not flip states",
    },
    {
