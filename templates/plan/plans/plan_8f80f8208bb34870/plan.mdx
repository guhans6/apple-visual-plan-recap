...
Replace the selected companion-artifact loading state with a true skeleton loader. The visual target is a loading surface that mirrors the real companion layout: canvas/review area above the document header and block rhythm. The loader itself contains no readable UI copy.
...
<ImplementationMap
  ...
  files={[
    {
      path: "templates/plan/app/pages/CompanionArtifactEditor.tsx",
    ...
    },
    {
      ...
      note: "Do not change the shared primitive. Compose the companion-specific shape in CompanionArtifactEditor so other templates do not inherit artifact-only behavior.",
      language: "tsx",
