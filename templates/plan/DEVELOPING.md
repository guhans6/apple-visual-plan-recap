# Visual Companion — Development Guide
...
server/                # Nitro API server
  routes/
    api/               # Route-only endpoints (uploads, webhooks, callbacks, streaming)
...
Use `server/routes/api/` only for protocols that cannot be modeled as JSON actions: multipart uploads, streaming/SSE/WebSocket, webhooks, callback/redirect flows, public SEO/OG endpoints, or binary/static asset serving. Do not add `/api/*` routes for normal CRUD, data queries, or pass-through wrappers around actions; the action endpoint already exists at `/_agent-native/actions/:name`.
