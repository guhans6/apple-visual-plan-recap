...
import { planSourceSchema, planStatusSchema } from "../server/plans.js";
...
export default defineAction({
  ...
  schema: z.object({
    ...
    visibility: z
      ...
      .default("org")
      .describe(
        "Visibility for the published recap. Defaults to 'org' so the recap is reviewable in the current org without becoming public.",
      ),
    ...
    sourceUrl: sourceUrlSchema.describe(
      "URL of the pull request, issue, or commit that this recap covers. Must be an http(s) URL.",
    ),
    idempotencyKey: z
      .string()
      .trim()
  ...
  mcpApp: {
    ...
    resource: embedApp({
      title: "Visual Recap",
      description:
        "Open the Visual Companion review surface for a visual code-review recap.",
