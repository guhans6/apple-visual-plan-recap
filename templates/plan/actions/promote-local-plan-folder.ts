...
export default defineAction({
  description:
    "Copy a DB-free local Visual Companion MDX folder into a repo-relative folder, using apps.plan.roots[0].path from agent-native.json when no targetPath is provided and falling back to plans/<slug>.",
  schema: z.object({
    slug: z
      .string()
      .min(1)
  ...
  link: ({ args }) => ({
    url: companionRoutePath(
      inferCompanionRouteKind(args.targetPath),
      args.slug,
      args.targetPath,
    ),
