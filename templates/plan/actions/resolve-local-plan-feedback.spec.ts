...
const action = (await import("./resolve-local-plan-feedback.js")).default as {
  run: (args: {
    slug: string;
    commentId: string;
    status: "resolved" | "open";
  }) => Promise<{ commentId: string; status: "resolved" | "open" }>;
};
const readAction = (await import("./get-local-plan-folder.js")).default as {
  run: (args: { slug: string }) => Promise<{
    comments: Array<{ id: string; status: string; resolvedAt: string | null }>;
  }>;
};

describe("resolve-local-plan-feedback action", () => {
  let tmpDir: string;
  let savedDir: string | undefined;
  let savedNodeEnv: string | undefined;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "plan-local-resolve-"));
  ...
  it("resolves and reopens local sidecar feedback without deleting history", async () => {
    await action.run({
      slug: "settings-flow",
      commentId: "fb_1",
      status: "resolved",
    });
    const resolved = await readAction.run({ slug: "settings-flow" });
    ...
    const reopened = await readAction.run({ slug: "settings-flow" });

    expect(reopened.comments[0]).toMatchObject({
      id: "fb_1",
      status: "open",
      resolvedAt: null,
    });
