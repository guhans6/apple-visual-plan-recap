...
export default defineAction({
  description:
    "Update a Visual Companion artifact's structured content blocks, prototype screens, sections, comments, or status. Prefer contentPatches for targeted edits. Use full content only for broad restructuring.",
  schema: z.object({
    ...
    comments: z
      ...
      .default([])
      .describe(
        "Legacy comment array retained for compatibility. Prefer the local companion feedback pipeline for new review work.",
      ),
    consumedCommentIds: z
      .array(z.string())
      .optional()
      .default([])
      .describe("Prefer the local companion feedback pipeline for marking feedback consumed."),
    note: z
      .string()
      .optional()
  ...
  mcpApp: {
    ...
    resource: embedApp({
      title: "Update Plan",
      description:
        "Open the Visual Companion editor for structured content, comments, and status changes.",
  ...
  run: async (args) => {
    ...
    const commentRequestEmail = onlyReviewerCommentWork
      ? isLocalPlanRuntime()
        ? getLocalPlanOwnerEmail()
    ...
    if (onlyReviewerCommentWork) {
      if (!commentRequestEmail) {
        throw new ForbiddenError(
          "Commenting on a plan requires local collaborator access.",
        );
      }
    ...
    await (async (tx: typeof db) => {
      ...
      //      content changed.
      // Comments still propagate because plan comments are queried independently
      // from the authored plan row (not via plans.updatedAt)
    ...
    const local = isLocalPlanRuntime()
      ...
      : null;
    const detailPath = local?.written
      ? local.routePath
      : planPath(bundle.plan.id, bundle.plan.kind);
    return {
      ...bundle,
      planId: bundle.plan.id,
