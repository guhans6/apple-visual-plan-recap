...
export default defineAction({
  ...
  mcpApp: {
    ...
    resource: embedApp({
      title: "Plan",
      description: "Open an imported Visual Companion review surface.",
  ...
  run: async (args) => {
    ...
    const local = isLocalPlanRuntime()
      ...
      : null;
    const detailPath = local?.written ? local.routePath : planPath(id, kind);
    return {
      ...bundle,
      planId: id,
      html: buildPlanHtml(bundle),
  ...
  link: ({ result }) => {
    const url = (result as { url?: string; plan?: { kind?: string } } | null)
      ?.url;
    const isRecap =
      (result as { plan?: { kind?: string } } | null)?.plan?.kind === "recap";
    if (!url) return null;
    return {
      url,
