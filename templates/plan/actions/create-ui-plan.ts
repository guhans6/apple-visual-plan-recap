...
export default defineAction({
  ...
  mcpApp: {
    ...
    resource: embedApp({
      title: "UI Plan",
      description:
        "Open the Visual Companion UI review surface for sketch mockups, state annotations, implementation maps, snippets, and validation notes.",
  ...
  run: async (args) => {
    ...
    return {
      ...
      fallbackInstructions:
        "Open the companion UI artifact, review the top pan/zoom sketch canvas when present, continue through the implementation-focused document blocks, add comments or drawings directly on the artifact, then read the pending companion feedback queue before implementing. Keep the artifact in repo-local companion files unless the user explicitly asks for a separate export or selection receipt.",
    };
  },
  link: ({ result }) => {
