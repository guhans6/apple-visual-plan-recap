...
export default defineAction({
  description:
    "See what the user is currently looking at in Visual Companion, including the active structured artifact, exported HTML, sections, and annotations.",
  schema: z.object({}),
  http: false,
  readOnly: true,
  run: async () => {
    ...
    if (nav?.localPlanSlug && isLocalPlanRuntime()) {
      ...
      try {
        ...
        screen.visualPlan = {
          ...
          agentWorkflow:
            "For fast companion prototype iteration, open the local companion artifact and use local companion feedback plus source patches or local-folder updates for targeted edits by blockId, prototype screenId, or canvas id. Prefer repo-local artifact changes over any hosted rollback or history flow.",
        };
      } catch {
