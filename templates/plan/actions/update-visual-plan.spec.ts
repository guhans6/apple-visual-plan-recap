...
describe("update-visual-plan comments", () => {
  ...
  it("rejects synthetic public-viewer identities before comment access is evaluated", async () => {
    request.email =
    ...
    ).rejects.toThrow("Plan plan_public not found");
    expect(resolveAccessMock).toHaveBeenCalledWith("plan", "plan_public", {
      userEmail: "local@agent-native.local",
    });
  });

  it("rejects synthetic guest identities before comment access is evaluated", async () => {
    request.email =
  ...
  it("returns a user-facing 403 when comment writes run outside local runtime with no request identity", async () => {
    ...
    await expect(
      ...
      statusCode: 403,
      message: "Commenting on a plan requires local collaborator access.",
    });
